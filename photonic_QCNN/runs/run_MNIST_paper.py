...
add1, add2 = 0, 0
nbr_class = 2
list_gates = PQNN(m // 2 + add1 + add2)
modes_detected = list(range(len(class_set)))
...
class Photonic_QCNN(nn.Module):
    ...
    def __init__(self, m, list_gates, modes_detected, device):
        ...
        self.measure = Measure_Photon_detection(
            2, 8 + add1 + add2, modes_detected, device
        )
...
from photonic_QCNN.training.training_paper import train_globally
...
for test in range(nbr_test):
    ...
    optimizer = torch.optim.Adam(network_dense.parameters(), lr=0.1, weight_decay=0.005)
    scheduler = ExponentialLR(optimizer, gamma=0.9)
    output_scale, train_epochs, test_interval = 120, 20, 1
