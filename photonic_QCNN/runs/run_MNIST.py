...
from photonic_QCNN.models.merlin_pqcnn import HybridModel
...
def run_experiment(
    ...
    """Run the complete experiment for one random state on MNIST"""
    print(f"Running experiment {run_id} with random state {random_state} on MNIST")
    ...
    if source == "paper":
        x_train, x_test, y_train, y_test = get_dataset("MNIST", "paper", 42)
        ...
        x_train, x_test, y_train, y_test = get_dataset("MNIST", "scratch", 42)
        x_train, x_test, y_train, y_test = convert_dataset_to_tensor(
            x_train, x_test, y_train, y_test
        )
        train_loader = convert_tensor_to_loader(x_train, y_train, batch_size=6)
    ...
    print(
        f"MNIST - Final train: {training_results['final_train_acc']:.4f}, test: {training_results['final_test_acc']:.4f}"
    )
...
def save_results(
    ...
    plt.tight_layout()
    plots_file = os.path.join(output_dir, "MNIST_training_plots.png")
...
def main():
    ...
    output_dir = f"../results/MNIST/{day}/{time_str}"
