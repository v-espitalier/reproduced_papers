...
def create_circuit(args):
    ...
    if args.circuit == "mzi":
        if args.train_circuit:
            left_side = pcvl.BS(theta=pcvl.P("theta_l_1"), phi_bl=pcvl.P("theta_l_2"), phi_tl=pcvl.P("theta_l_3"), phi_br=pcvl.P("theta_l_4"), phi_tr=pcvl.P("theta_l"))
            right_side = pcvl.BS(theta=pcvl.P("theta_r_1"), phi_bl=pcvl.P("theta_r_2"), phi_tl=pcvl.P("theta_r_3"), phi_br=pcvl.P("theta_r_4"), phi_tr=pcvl.P("theta_r"))
        else:
            left_side = pcvl.BS()
            right_side = pcvl.BS()
        ...
        if args.train_circuit:
            left_side = pcvl.GenericInterferometer(2,
                                    lambda i: pcvl.BS(theta=pcvl.P(f"theta_l1{i}_bs"), phi_tr=pcvl.P(f"theta_l2{i}_bs"), phi_br=pcvl.P(f"theta_l3{i}_bs"), phi_tl=pcvl.P(f"theta_l4{i}_bs"), phi_bl=pcvl.P(f"theta_l5{i}_bs")) // pcvl.PS(phi=pcvl.P(f"theta_psl1{i}")) // \
                                              pcvl.BS(theta=pcvl.P(f"theta_l6{i}_bs"), phi_tr=pcvl.P(f"theta_l7{i}_bs"), phi_br=pcvl.P(f"theta_l8{i}_bs"), phi_tl=pcvl.P(f"theta_l9{i}_bs"), phi_bl=pcvl.P(f"theta_l10{i}_bs")) // pcvl.PS(phi=pcvl.P(f"theta_{i}")),
                                    shape=pcvl.InterferometerShape.RECTANGLE)
            right_side = pcvl.GenericInterferometer(2,
                                    lambda i: pcvl.BS(theta=pcvl.P(f"theta_r1{i}_bs"), phi_tr=pcvl.P(f"theta_r2{i}_bs"), phi_br=pcvl.P(f"theta_r3{i}_bs"), phi_tl=pcvl.P(f"theta_r4{i}_bs"), phi_bl=pcvl.P(f"theta_r5{i}_bs")) // pcvl.PS(phi=pcvl.P(f"theta_psr1{i}")) // \
                                              pcvl.BS(theta=pcvl.P(f"theta_r6{i}_bs"), phi_tr=pcvl.P(f"theta_r7{i}_bs"), phi_br=pcvl.P(f"theta_r8{i}_bs"), phi_tl=pcvl.P(f"theta_r9{i}_bs"), phi_bl=pcvl.P(f"theta_r10{i}_bs")) // pcvl.PS(phi=pcvl.P(f"theta_psr2{i}")),
                                    shape=pcvl.InterferometerShape.RECTANGLE)
        else:
            left_side = pcvl.GenericInterferometer(2, lambda i: pcvl.BS() // pcvl.PS(phi=random.random() * np.pi),
                                                   shape=pcvl.InterferometerShape.RECTANGLE)
            right_side = pcvl.GenericInterferometer(2, lambda i: pcvl.BS() // pcvl.PS(phi=random.random() * np.pi),
                                                   shape=pcvl.InterferometerShape.RECTANGLE)
    elif args.circuit == "general":
        if args.train_circuit:
            left_side = pcvl.GenericInterferometer(2,
                                    lambda i: pcvl.BS(theta=pcvl.P(f"theta_l1{i}_bs")) // pcvl.PS(phi=pcvl.P(f"theta_psl1{i}")) // \
                                              pcvl.BS(theta=pcvl.P(f"theta_l2{i}_bs")) // pcvl.PS(phi=pcvl.P(f"theta_{i}")),
                                    shape=pcvl.InterferometerShape.RECTANGLE)
            right_side = pcvl.GenericInterferometer(2,
                                    lambda i: pcvl.BS(theta=pcvl.P(f"theta_r1{i}_bs")) // pcvl.PS(phi=pcvl.P(f"theta_psr1{i}")) // \
                                              pcvl.BS(theta=pcvl.P(f"theta_r2{i}_bs")) // pcvl.PS(phi=pcvl.P(f"theta_psr2{i}")),
                                    shape=pcvl.InterferometerShape.RECTANGLE)
        else:
            left_side = pcvl.GenericInterferometer(2, lambda i: pcvl.BS() // pcvl.PS(phi=random.random() * np.pi),
                                                   shape=pcvl.InterferometerShape.RECTANGLE)
        ...
        if args.train_circuit:
            left_side = pcvl.GenericInterferometer(2,
                                    lambda i: pcvl.BS() // pcvl.PS(phi=pcvl.P(f"theta_l1{i}")) // \
                                              pcvl.BS() // pcvl.PS(phi=pcvl.P(f"theta_{i}")),
                                    shape=pcvl.InterferometerShape.RECTANGLE)
            right_side = pcvl.GenericInterferometer(2,
                                    lambda i: pcvl.BS() // pcvl.PS(phi=pcvl.P(f"theta_r1{i}")) // \
                                              pcvl.BS() // pcvl.PS(phi=pcvl.P(f"theta_r2{i}")),
                                    shape=pcvl.InterferometerShape.RECTANGLE)
        else:
            left_side = pcvl.GenericInterferometer(2, lambda i: pcvl.BS() // pcvl.PS(phi=random.random() * np.pi),
                                                   shape=pcvl.InterferometerShape.RECTANGLE)
        ...
        if args.train_circuit:
            left_side = pcvl.GenericInterferometer(2,
                                    lambda i: pcvl.BS(theta=pcvl.P(f"theta_l1{i}_bs")) // pcvl.PS(phi=random.random() * np.pi) // \
                                              pcvl.BS(theta=pcvl.P(f"theta_l2{i}_bs")) // pcvl.PS(phi=random.random() * np.pi),
                                    shape=pcvl.InterferometerShape.RECTANGLE)
            right_side = pcvl.GenericInterferometer(2,
                                    lambda i: pcvl.BS(theta=pcvl.P(f"theta_r1{i}_bs")) // pcvl.PS(phi=random.random() * np.pi) // \
                                              pcvl.BS(theta=pcvl.P(f"theta_r2{i}_bs")) // pcvl.PS(phi=random.random() * np.pi),
                                    shape=pcvl.InterferometerShape.RECTANGLE)
        else:
            left_side = pcvl.GenericInterferometer(2, lambda i: pcvl.BS() // pcvl.PS(phi=random.random() * np.pi),
                                                   shape=pcvl.InterferometerShape.RECTANGLE)
