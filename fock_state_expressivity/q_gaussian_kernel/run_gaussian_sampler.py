...
from training import save_models, train_models_multiple_runs, visualize_learned_function
        ...
        for betas in betas_s:
            for weight_decay in weight_decays:
                for lr in lrs:
                    ...
                    all_results, unique_id = train_models_multiple_runs(
                        num_photons, colors, delta, ys_info, args
                    )
                    visualize_learned_function(
                        all_results,
                        num_photons,
                        x_on_pi,
                        delta,
...
visualize_learned_function(
    all_results, num_photons, x_on_pi, delta, ys_info, args, unique_id
)
save_models(all_results, num_photons, ys_info)

final_time = time.time()
print(
    f"Total time to train {args.num_runs * len(num_photons) * len(std_names)} models and save "
