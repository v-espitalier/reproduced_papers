...
def visu_target_functions(x_on_pi, ys):
    ...
    for y, sigma in zip(ys, sigmas):
        axis = axs[i // 2][j % 2]
        axis.scatter(x_on_pi, y, label=rf"$\sigma$={sigma}", s=10, color="k")
...
def get_visual_sample(x, y, x_test, y_test, title):
    ...
    axs[0].scatter(
        x[0][:, 0], x[0][:, 1], c=y[0], cmap="bwr", marker="o", label="Train set"
    )
    axs[0].scatter(
        x_test[0][:, 0],
        x_test[0][:, 1],
    ...
    # axs[0].legend()
    axs[1].scatter(
        x[1][:, 0], x[1][:, 1], c=y[1], cmap="bwr", marker="o", label="Train set"
    )
    axs[1].scatter(
        x_test[1][:, 0],
        x_test[1][:, 1],
    ...
    # axs[1].legend()
    axs[2].scatter(
        x[2][:, 0], x[2][:, 1], c=y[2], cmap="bwr", marker="o", label="Train set"
    )
    axs[2].scatter(
        x_test[2][:, 0],
        x_test[2][:, 1],
