...
import wandb
from classical_models import count_svm_parameters, get_mlp_deep, get_mlp_wide
from data import (
    get_circle,
    get_linear,
    get_moon,
...
from VQC import count_parameters, get_vqc
...
def prepare_data(get_visu=False):
    ...
    if get_visu:
        get_visual_sample(
            torch.cat((x_lin_train, x_lin_test)),
            torch.cat((y_lin_train, y_lin_test)),
            "Data visualization of linear dataset",
        )
        get_visual_sample(
            torch.cat((x_circ_train, x_circ_test)),
            torch.cat((y_circ_train, y_circ_test)),
            "Data visualization of circular dataset",
        )
        get_visual_sample(
            torch.cat((x_moon_train, x_moon_test)),
            torch.cat((y_moon_train, y_moon_test)),
            "Data visualization of moon dataset",
        )
    ...
    save_dataset_locally(
        "data/linear_dataset_vqc.pt", x_lin_train, x_lin_test, y_lin_train, y_lin_test
    )
    save_dataset_locally(
        "data/circular_dataset_vqc.pt",
        x_circ_train,
        x_circ_test,
        y_circ_train,
        y_circ_test,
    ...
    save_dataset_locally(
        "data/moon_dataset_vqc.pt", x_moon_train, x_moon_test, y_moon_train, y_moon_test
    )
...
def train_model(
    model, x_train, y_train, x_test, y_test, model_name, args, extra_visu=False
):
...
def train_model_multiple_runs(type, args):
    ...
    try:
        ...
        print(
            "Datasets not found, generating data and saving it locally at ./data/...\n"
        )
    ...
    for i, dataset in enumerate(datasets):
        ...
        if type == "vqc":
            ...
            for run in range(args.num_runs):
                ...
                run_results = train_model(
                    model, x_train, y_train, x_test, y_test, f"VQC-run{run + 1}", args
                )
                models.append(model)
            ...
            for run in range(args.num_runs):
                ...
                args.set_dataset_name(dataset)
                run_results = train_model(
                    model, x_train, y_train, x_test, y_test, f"MLP-run{run + 1}", args
                )
                models.append(model)
                model_runs.append(run_results)
            ...
            for run in range(args.num_runs):
                ...
                print(
                    f"  Run {run + 1}/{args.num_runs}, SVM has {num_params} parameters..."
                )
                args.set_dataset_name(dataset)
        ...
        visualize_decision_boundary(
            type, best_model, x_train, y_train, x_test, y_test, best_acc, args
        )
    ...
    for dataset_name, model_data in results.items():
        ...
        if args.log_wandb:
            ...
            wandb.log(
                {
                    "Mean final test accuracy:": avg_acc,
                    "Final test accuracy std": std_acc,
                }
            )
            wandb.finish()
...
def visualize_decision_boundary(
    type, model, x_train, y_train, x_test, y_test, acc, args, resolution=100
):
    ...
    plt.title(
        f"Decision Boundary of {args.model_type}\non {args.dataset_name} dataset",
        size=16,
    )
    ...
    plt.savefig(
        f"./results/decision_boundary_{args.model_type}_{args.dataset_name}.png"
    )
    plt.clf()
    return
...
def visualize_results(results):
    ...
    # Customize plots
    for ax, title in zip(
        [ax1, ax2, ax3], ["Training Loss", "Training Accuracy", "Test Accuracy"]
    ):
