...
def visu_accuracies_param(models, parameters, accuracy_mean, accuracy_std, dataset_str):
    ...
    ax1.set_title(
        f"Mean Final Test Accuracy (10 runs) on {dataset_str} Dataset",
        fontsize=14,
        fontweight="bold",
    )
    ...
    for _i, (bar, mean_val, std_val) in enumerate(
        zip(bars1, accuracy_mean, accuracy_std)
    ):
    ...
    bars2 = ax2.bar(
        x_pos, parameters, alpha=0.7, color="lightcoral", edgecolor="darkred"
    )
    ax2.set_ylabel("Number of Parameters", fontsize=12)
    ax2.set_title(
        "Model Complexity (Number of Parameters)", fontsize=14, fontweight="bold"
    )
    ...
    ax.set_title(
        f"Model Performance vs Complexity on {dataset_str} Dataset",
        fontsize=14,
        fontweight="bold",
    )
...
def regroup_figures():
    ...
    # Set the main title
    fig.suptitle(
        "Decision boundary of VQC in function of dataset and input Fock state",
        fontsize=30,
        fontweight="bold",
        y=0.98,
    )
    ...
    plt.subplots_adjust(
        hspace=-0.1, wspace=0.0, top=0.90, bottom=0.02, left=0.02, right=0.98
    )
