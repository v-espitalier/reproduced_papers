...
import tqdm
import wandb
from approx_kernel import (
    get_approx_kernel_predict,
    get_approx_kernel_train,
    get_q_model,
    get_x_r_i_s,
    get_z_s_classically,
)
...
def get_save_data(args):
    ...
    x_train, x_test, y_train, y_test = split_train_test(
        x, y, args.test_prop, args.random_state
    )
...
def training_q_model(x_train, x_test, args):
    ...
    for _epoch in epoch_bar:
        ...
        for x_batch, y_batch in dataloader:
            ...
            optimizer.zero_grad()
            # Reformat input
            x_batch = x_batch.view(args.batch_size * args.r, -1) * torch.tensor(
    ...
    print(
        f"Mean gradient per optimization step per parameter: {mean_grad_per_param:.6f}"
    )
