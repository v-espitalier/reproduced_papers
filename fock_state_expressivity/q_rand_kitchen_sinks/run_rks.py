import numpy as np
from training import get_save_data, q_rand_kitchen_sinks, train_svm, classical_rand_kitchen_sinks
from hyperparameters import Hyperparameters
from approx_kernel import get_random_w_b
import os
os.environ["WANDB_SILENT"] = "true"
import wandb
from utils import combine_saved_figures, save_hyperparameters, create_experiment_dir
...
def run_single_gamma_r(args):
    ...
    q_model_opti, q_kernel_matrix_train, q_kernel_matrix_test = q_rand_kitchen_sinks(X_train, X_test, y_train, args)
    q_acc = train_svm(q_kernel_matrix_train, q_kernel_matrix_test, q_model_opti, X_train, X_test, y_train, y_test,
                      args)
    ...
    kernel_matrix_train, kernel_matrix_test = classical_rand_kitchen_sinks(X_train, X_test, y_train, args)
    acc = train_svm(kernel_matrix_train, kernel_matrix_test, None, X_train, X_test, y_train, y_test,
                    args)
...
test_prop=0.4
num_photon=10
output_mapping_strategy='LINEAR'  # ['NONE', 'LINEAR', 'GROUPING'] GROUPING does not work in this context
no_bunching=False
...
args = Hyperparameters(n_samples=n_samples, noise=noise, random_state=random_state, scaling=scaling,
                       test_prop=test_prop, num_photon=num_photon, output_mapping_strategy=output_mapping_strategy,
                       no_bunching=no_bunching, circuit=circuit, batch_size=batch_size, optimizer=optimizer,
                       learning_rate=learning_rate, betas=betas, weight_decay=weight_decay, num_epochs=num_epochs, C=C,
                       r=r, gamma=gamma, train_hybrid_model=train_hybrid_model, pre_encoding_scaling=pre_encoding_scaling,
                       z_q_matrix_scaling=z_q_matrix_scaling, hybrid_model_data=hybrid_model_data)
